...
folder = "" # Path to the folder containing the data and targets (.xlsx)
df_train = pd.read_excel(os.path.join(folder, ".xlsx"), header=None) # Filename of the training targets (.xlsx)
num_samples_train = df_train.shape[0]
configs_train = df_train.iloc[:, 1:n_labels + 1].values

file_list_train = sorted(glob.glob(os.path.join(folder, "", "*.txt")), # Folder of the training targets
...
df_test = pd.read_excel(os.path.join(folder, ""), header=None) # Filename of the test targets (.xlsx)
num_samples_test = df_test.shape[0]
configs_test = df_test.iloc[:, 1:n_labels + 1].values

file_list_test = sorted(glob.glob(os.path.join(folder, "", "*.txt")),  # Folder of the test targets
                        key=lambda x: int(os.path.basename(x).split(".")[0]))
...
# MAE 
mae_x = np.mean(np.abs(y_pred[:, :5] - configs_test[:, :5]), axis=0)
mae_y = np.mean(np.abs(y_pred[:, 5:10] - configs_test[:, 5:10]), axis=0)
mae_z = np.mean(np.abs(y_pred[:, 10:15] - configs_test[:, 10:15]), axis=0)
